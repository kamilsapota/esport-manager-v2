...
import { Player, MatchResult, Team, PlayerRole } from '../types';
...
export const generateFreeAgents = async (count: number, budget: number): Promise<Player[]> => {
  ...
  Use realistic nicknames and countries.`;
  ...
  try {
    ...
    return rawPlayers.map((p: any) => ({
      ...
      avatarSeed: p.alias // Use alias as seed for consistent avatars if we had an avatar gen
    }));
  } catch (error) {
...
const getTeamContext = (team: Team): string => {
  ...
  const rosterList = team.players.map(p => 
    `- ${p.alias} (${p.country}, ${p.role}): OVR ${Math.round((p.stats.aim + p.stats.reflex + p.stats.strategy)/3)}`
  ).join('\n');
